...
from paths import CANTON_FOOTPRINT_CSVS_DIR, FOOTPRINT_DATA_DIR, REGBL_DATA_DIR
...
# DATA TABLE
def download_regbl(NROWS: int = None) -> pd.DataFrame:
    ...
    def read_regbl_data(extraction_dir: str, nrows: int = NROWS) -> pd.DataFrame:
        ...
        building = pd.read_csv(building_file, nrows=nrows, sep="\t", low_memory=False)
        return building
    ...
    for regbl_zip_file, command in commands.items():
...
def dowload_footprint_data(force_download: bool = False):
    ...
    for alias in VALID_CANTON_ALIAS:
        ...
        # from IPython import embed; embed(header="sto"); exit()
        print(f"Dowload footprint file : {alias}")
...
def read_footprint_data(gdb_directory, layer: str) -> gpd.GeoDataFrame:
    """Turn GDB directory into GeoDataFrame (for a specific layer)."""
    layers = fiona.listlayers(gdb_directory)
    if layer not in layers:
        # data = {
        #     "GRUND_AENDERUNG": list(),
        #     "HERKUNFT": list(),
        #     "HERKUNFT_MONAT": list(),
        #     "HERKUNFT_JAHR": list(),
        #     "ORIGINAL_HERKUNFT": list(),
        #     "ERSTELLUNG_MONAT": list(),
        #     "ERSTELLUNG_JAHR": list(),
        #     "REVISION_JAHR": list(),
        #     "REVISION_MONAT": list(),
        #     "DATUM_ERSTELLUNG": list(),
        #     "GELAENDEPUNKT": list(),
        #     "GESAMTHOEHE": list(),
        #     "EGID": list(),
        # }
    ...
    # from IPython import embed; embed(header="sto"); exit()
    # print(layers)
    # raise NotImplemented(gdb_directory)
...
def extract_polygon(
    ...
    # from IPython import embed; embed(header="test-test"); exit()
...
def spatial_join(footprint: gpd.GeoDataFrame, regbl: gpd.GeoDataFrame, prefix_fd: str):
    ...
    regbl = gpd.GeoDataFrame(regbl, geometry=geometry)
    ...
    spatial["levl_code"] = None
    spatial["cntr_code"] = None
    spatial["name_engl"] = None
    spatial["name"] = None
    return spatial
...
def set_spatial_table():
    ...
    for gdb_file in tqdm(gdb_files):
        ...
        spatial = spatial_join(
            footprint=dataframe, 
            regbl=regbl, 
            prefix_fd=PREFIX_FID
        )
        spatials.append(spatial)
    
    from IPython import embed
    embed(header="end of the project")
    exit()
